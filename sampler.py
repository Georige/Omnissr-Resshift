...
class ResShiftSampler(BaseSampler):
    def sample_func(self, y0, noise_repeat=False, mask=False):
        ...
        if not (ori_h % offset == 0 and ori_w % offset == 0):
            ...
            flag_pad = False
        # cond_lq是低清图条件，mask是掩码。model_kwargs是传入模型的关键字，通常里面有当前图片和当前时间，然后后另外加上这mask和lq
        if self.configs.model.params.cond_lq and mask is not None: # 如果都是非空
            model_kwargs={
                    'lq':y0,
                    'mask': mask,
                    }
        elif self.configs.model.params.cond_lq: #如果有cond_lq
            model_kwargs={'lq':y0,}
        else:
            model_kwargs = None # 啥也没有

        results = self.base_diffusion.p_sample_loop( # 完成整个DDPM采样过程，直接得到结果，并且解码成像素
                y=y0,
                model=self.model,
                first_stage_model=self.autoencoder,
                noise=None,
                noise_repeat=noise_repeat,
                clip_denoised=(self.autoencoder is None), # 需不需要限制采样之后范围
                denoised_fn=None,
        ...
        if flag_pad:
            results = results[:, :, :ori_h*self.sf, :ori_w*self.sf] # 裁剪去掉填充的像素
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=True, bs=1, noise_repeat=False):
        ...
        Input:
            ...
            bs: int, default bs=1, bs % num_gpus == 0 # 同时处理多少张图片,一个GPU一张
            mask_path: image mask for inpainting
        '''
        def _process_per_image(im_lq_tensor, mask=None): # 不管进来的图有多大、有没有遮罩（Mask），都要想办法把它喂给显卡吃下去，并且保证吐出来的结果是一张完整、无缝的高清大图。
            '''
            Input:
            ...
            if im_lq_tensor.shape[2] > self.chop_size or im_lq_tensor.shape[3] > self.chop_size:
                ...
                for im_lq_pch, index_infos in im_spliter: # 对大图进行切块
                    if mask is not None:
                        im_lq_pch, mask_pch = im_lq_pch[:, :-1], im_lq_pch[:, -1:,]
                    else:
                        mask_pch = None
                    with context():
                        im_sr_pch = self.sample_func( # 每一小块做采样
                                im_lq_pch,
                                noise_repeat=noise_repeat,
                                mask=mask_pch,
                                )     # 1 x c x h x w, [-1, 1]
                    im_spliter.update(im_sr_pch, index_infos)
                im_sr_tensor = im_spliter.gather() # 处理完之后将所有小块拼成大图
            else:
                print(im_lq_tensor.shape)
                with context():
                    im_sr_tensor = self.sample_func( # 不切块直接处理整图
                            im_lq_tensor,
                            noise_repeat=noise_repeat,
                            mask=mask,
                            )     # 1 x c x h x w, [-1, 1]

            im_sr_tensor = im_sr_tensor * 0.5 + 0.5 # 把数据变回[0,1]，这是数据保存喜欢的
            if mask_back and mask is not None: # 如果是inpaint任务，做最后一步
                mask = mask * 0.5 + 0.5
                im_lq_tensor = im_lq_tensor * 0.5 + 0.5
                im_sr_tensor = im_sr_tensor * mask + im_lq_tensor * (1 - mask)
            return im_sr_tensor

        in_path = Path(in_path) if not isinstance(in_path, Path) else in_path # 如果不是path对象，转换成path对象
        out_path = Path(out_path) if not isinstance(out_path, Path) else out_path

        if self.rank == 0:
            assert in_path.exists()  # 输出文件夹必须存在，否则报错
            if not out_path.exists():
                out_path.mkdir(parents=True) # 创建输出文件夹

        if self.num_gpus > 1:
            dist.barrier() # 全员集结，集结所有gpu

        # 分配图纸 data config
        if in_path.is_dir():
            if mask_path is None:
                data_config = {'type': 'base', # 普通模式
                               'params': {'dir_path': str(in_path), # 图片位置
                                          'transform_type': 'default',
                                          'transform_kwargs': { # 归一化参数
                                              'mean': 0.5,
                                              'std': 0.5,
                                              },
                                          'need_path': True,
                                          'recursive': True, # 递归寻找子目录
                                          'length': None,
                                          }
                               }
            else:
                data_config = {'type': 'inpainting_val', # 修复数据模式
                               'params': {'lq_path': str(in_path), # 待修复图片路径
                                          'mask_path': mask_path, # 遮盖图路径
                                          'transform_type': 'default',
                                          'transform_kwargs': {
                                              'mean': 0.5,
                                              'std': 0.5,
            ...
            dataset = create_dataset(data_config) # 数据采集，将数据装配好
            self.write_log(f'Find {len(dataset)} images in {in_path}') # 清点库存
            dataloader = torch.utils.data.DataLoader(
                    dataset, # 从哪里拿数据
                    batch_size=bs, # 一次拿多少数据
                    shuffle=False, # 顺序要不要打乱
                    drop_last=False, # 最后装不满要不要扔掉
                    )
            for data in dataloader: # 手动实现数据并行
                micro_batchsize = math.ceil(bs / self.num_gpus) # bs = 4，gpus = 2，一次加载四张图，四张图同时出现在两个gpu里面
                ind_start = self.rank * micro_batchsize
                ind_end = ind_start + micro_batchsize
                micro_data = {key:value[ind_start:ind_end] for key,value in data.items()}# 让gpu1处理1 2图，让gpu2处理3 4图

                if micro_data['lq'].shape[0] > 0: # 只有拿到gpu的人才干活
                    ...
                    for jj in range(results.shape[0]):
                        im_sr = util_image.tensor2img(results[jj], rgb2bgr=True, min_max=(0.0, 1.0)) #[0,1] 转换成 255
                        im_name = Path(micro_data['path'][jj]).stem # 获取原始文件名
                        im_path = out_path / f"{im_name}.png" # 最终保存地址
                        util_image.imwrite(im_sr, im_path, chn='bgr', dtype_in='uint8') # 每个GPU拿到自己生成的高清图然后保存
            if self.num_gpus > 1:
                dist.barrier() # 强制同步，一起收工
        else: # 只处理一张图片
            im_lq = util_image.imread(in_path, chn='rgb', dtype='float32')  # h x w x c 读图片像素
            im_lq_tensor = util_image.img2tensor(im_lq).cuda()              # 1 x c x h x w 变成pytorch张量形式，并且搬到gpu上
            if mask_path is not None: # 处理Mask
            ...
            im_sr_tensor = _process_per_image(
                    (im_lq_tensor - 0.5) / 0.5, # 把数据归一化到[-1,1]
                    mask=(im_mask_tensor - 0.5) / 0.5 if mask_path is not None else None,
                    )

            im_sr = util_image.tensor2img(im_sr_tensor, rgb2bgr=True, min_max=(0.0, 1.0)) # 从[-1,1]变回0-255
            im_path = out_path / f"{in_path.stem}.png"
            util_image.imwrite(im_sr, im_path, chn='bgr', dtype_in='uint8') # 保存到硬盘
