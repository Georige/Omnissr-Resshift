...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, **kwargs):
        self.use_timesteps = set(use_timesteps) # 用户制定的步数
        self.timestep_map = []
        self.original_num_steps = len(kwargs["sqrt_etas"])

        base_diffusion = GaussianDiffusion(**kwargs)  # pylint: disable=missing-kwoa 创建一个完整的1000步扩散作为参考
        new_sqrt_etas = []
        for ii, etas_current in enumerate(base_diffusion.sqrt_etas): # 遍历完整的1000步
            if ii in self.use_timesteps: # 只保留想要的步数
                new_sqrt_etas.append(etas_current)
                self.timestep_map.append(ii) # 记录映射关系
        kwargs["sqrt_etas"] = np.array(new_sqrt_etas) # 把参数替换成缩减之后的步数
        super().__init__(**kwargs) # 用缩短后的参数初始化父类
