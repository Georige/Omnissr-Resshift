...
class DummyModel(nn.Module):
    ...
    def _gaussian_weights(self, tile_width, tile_height, nbatches):
        ...
        return torch.tile(torch.tensor(weights, device=self.device), (nbatches, self.channels, 1, 1))
...
def main():
    ...
    model.device = device
    model.channels = 3 # 潜空间通道数，ResShift是3 Omnissr是4
    # 缩小函数
    with torch.no_grad():
        factor = int(args_res.scale)
    ...
    with torch.no_grad():
        ...
        #         noise_repeat=False
        #         ) 
        pers_in = pers_in.to(device)
        stacked_tensor = resshift_sampler.sample_func(y0 = pers_in)
