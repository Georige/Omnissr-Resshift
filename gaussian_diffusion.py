...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x_t.shape[:2]
        assert t.shape == (B,)
        model_output = model(self._scale_input(x_t, t), t, **model_kwargs) # scale_input是将数据预处理，把输入的方差强行拉回到一个标准范围，model使用模型做预测

        model_variance = _extract_into_tensor(self.posterior_variance, t, x_t.shape) # 查后验方差表，并广播成和图片一样大的形状
        ...
        model_mean, _, _ = self.q_posterior_mean_variance( # 后验分布，算出下一个分布的均值和方差
    ...
    def p_sample(self, model, x, y, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        ...
        """
        out = self.omnissr_p_mean_variance( # 已经更改了换成omnissr的核心函数
            model,
            x, # 这个是潜变量稍微模糊一点
            y, # 这个是原图的潜变量
            t,
        ...
        if noise_repeat: # 如果noise_repeat = T，则同一个批次都添加相同的噪声，拥有相同的命运
            noise = noise[0,].repeat(x.shape[0], 1, 1, 1)
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
        )  # no noise when t == 0 一旦到达最后一步，加噪系数归零
    ...
    def p_sample_loop(
        ...
        noise=None,
        noise_repeat=False, # 强制同一个batch里面的图片
        clip_denoised=True, # 强制把模型预测的图片像素限制在 [-1,1]之间，防止数值爆炸
        denoised_fn=None, # 在上一轮模型预测出来的原图中，使用这个函数修改原图，然后传入下一轮的采样计算
        model_kwargs=None,  # 存放初始原图作为条件，还有掩码mask
        device=None,
        progress=False,
    ):
        """
    ...
    def p_sample_loop_progressive(
        ...
        z_sample = self.prior_sample(z_y, noise) # 把原图弄脏一点点

        indices = list(range(self.num_timesteps))[::-1] #range计数变成列表，然后将列表反转，从而实现从后往前取
        if progress: # 是否加入进度条
        ...
        for i in indices:
            t = th.tensor([i] * y.shape[0], device=device) # 做一个y一维张量 [i] * batch
            with th.no_grad():
                out = self.p_sample(
                    model,
                    z_sample, # 这个是在潜变量的基础上再弄脏一点点
                    z_y, # 进去采样的时候已经是潜变量了
                    t,
                    clip_denoised=clip_denoised,
                    denoised_fn=denoised_fn,
                ...
                yield out # 算出一步out就return，暂停函数，等待下一次调用重新下一步
                z_sample = out["sample"]
    ...
    def decode_first_stage(self, z_sample, first_stage_model=None, consistencydecoder=None):
        ...
        if first_stage_model is None:
            ...
            if consistencydecoder is None:
                out = decoder(z_sample.type(model_dtype)) # 让潜空间的数据类型精度对齐模型参数精度，否则会报错
            else:
                with th.cuda.amp.autocast(): # 如果使用consistencydecoder的话，启用混合精度模型，系统自己判断精度
                    out = decoder(z_sample)
            if not model_dtype == data_dtype: # 还原数据原来的精度
    ...
    def encode_first_stage(self, y, first_stage_model, up_sample=False):
        ...
        if up_sample and self.sf != 1:
            y = th.nn.functional.interpolate(y, scale_factor=self.sf, mode='bicubic')
        ...
        if first_stage_model is None:
            ...
            with th.no_grad(): # 确保编码时不计算梯度，节省显存
                # =========================================================
                # 核心防爆显存机制：Chunking (分块处理)
                # =========================================================
                batch_size = y.shape[0]
                # chunk_size 控制每次送入显卡的切片数。
                # 如果设为 2 依然爆显存，请将其改为 1。
                chunk_size = 1 
                out_chunks = []
                
                for i in range(0, batch_size, chunk_size):
                    # 1. 切片：每次只取 chunk_size 张图
                    y_chunk = y[i : i + chunk_size]
                    
                    # 2. 编码：将这一小撮图送入 VAE 潜空间
                    z_y_chunk = first_stage_model.encode(y_chunk)
                    
                    # 3. 兼容保护：某些版本的 LDM encode 返回的是分布对象，需提取张量
                    if hasattr(z_y_chunk, 'sample'):
                        z_y_chunk = z_y_chunk.sample()
                    elif hasattr(z_y_chunk, 'mode'):
                        z_y_chunk = z_y_chunk.mode()
                        
                    # 4. 缩放潜变量并存入临时列表
                    out_chunk = z_y_chunk * self.scale_factor
                    out_chunks.append(out_chunk)
                    
                # 5. 组装：沿着批次维度 (dim=0) 把所有处理好的小块拼回一个完整的张量
                # 这时它已经是体积很小的潜变量了，拼接起来完全不会爆显存
                out = th.cat(out_chunks, dim=0)
                # =========================================================
    ...
    def omnissr_p_mean_variance(
        ...
        :param model: the model, which takes a signal and a batch of timesteps
                      as input.
        :param x_t: the [N x C x ...] tensor at time t. 18张切片 当前时刻的潜变量
        :param y: the [N x C x ...] tensor of degraded inputs. 18张切片 原图的潜变量
        :param t: a 1-D Tensor of timesteps.
        :param clip_denoised: if True, clip the denoised signal into [-1, 1].
        :param denoised_fn: if not None, a function which applies to the
            x_start prediction before it is used to sample. Applies before
        ...
        model_output = model(self._scale_input(x_t,t), t , **model_kwargs) # 模型输出是当前图片到清晰原图之间的差距
        ...
        old_x0 = pred_xstart
        x0_pers = rearrange(old_x0, '(B Np) C H W -> B C H W Np', Np = self.num_tan_patch) # 切片数量 
        
        img_outputs = []
        
        ## 解码
        for idx in range(self.num_tan_patch):
            img_pers = self.decode_first_stage(x0_pers[..., idx])
            img_outputs.append(img_pers)
            
        img_pers = torch.stack(img_outputs, dim = -1) # [B 3 512 512 Np] 
        img_pers = torch.clamp(img_pers, min = -1., max = 1.)
        
        ## 18切片合成全景图
        weighted_img_pers = img_pers * self.img_tan_weights_pers # [B C H W Np]
        weighted_img_erp = self.multi_viewer.pers2equi(weighted_img_pers, fov = self.fov, nrows = self.nrows, erp_size = self.hr_erp_size, pre_upscale = self.pre_upscale, pre_mode = 'bicubic') 
        weighted_tan_erp = self.multi_viewer.pers2equi(self.img_tan_weights_pers, fov = self.fov, nrows = self.nrows, erp_size = self.hr_erp_size, pre_upscale = self.pre_upscale, pre_mode = 'bicubic') 
        img_erp = weighted_img_erp / weighted_tan_erp
        img_erp = torch.clamp(img_erp, min = -1., max = 1.)
        
        ## 做缩小契合
        if self.gamma_e != 0: 
            new_img_erp = range_null_space_decomposition(x0 = img_erp, y = self.lr_erp, H_funcs = self.H_funcs) 
            new_img_erp = (1 - self.gamma_e) * img_erp + (self.gamma_e) * new_img_erp
        else:
            new_img_erp = img_erp
            
        ## 重新再做切片
        new_img_pers, _, _, _ = self.multi_viewer.equi2pers(new_img_erp, fov = self.fov, nrows = self.nrows, patch_size = (img_pers.shape[2], img_pers.shape[3]), per_upscale = self.pre_upscale, pre_mode = 'bicubic') 
        new_img_pers = new_img_pers.cuda() # 搬进GPU
        
        ## 切片编码回潜变量
        new_img_pers = torch.clamp(new_img_pers, min = -1, max = 1) # 限制到[-1,1]
        new_x0_pers = [] # 储存切片潜变量
        for idx in range(self.num_tan_patch):
            new_x0 = self.encode_first_stage(new_img_pers[...,idx])
            new_x0_pers.append(new_x0)
        new_x0_pers = torch.stack(new_x0_pers,dim = -1) 
        new_x0 = rearrange(new_x0_pers, "B C H W Np -> (B Np) C H W", Np = self.num_tan_patch) 
        x0 = (1 - self.gamma_l) * old_x0 + (self.gamma_l) * new_x0 
        ...
        assert (
            model_mean.shape == model_log_variance.shape == x0.shape == x_t.shape
        )
        return {
            "mean": model_mean,
            "variance": model_variance,
...
class GaussianDiffusionDDPM:
    ...
    def q_sample(self, x_start, t, noise=None): # 向前过程
        """
        Diffuse the data for a given number of diffusion steps.
    ...
    def ddim_sample(
        ...
        eps = self._predict_eps_from_xstart(x, t, out["pred_xstart"]) # 反推出噪声
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape) # 当前图像的保留比例
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, x.shape) # 下一个时刻图像的保留比例
        sigma = (
            eta
            * th.sqrt((1 - alpha_bar_prev) / (1 - alpha_bar))
            * th.sqrt(1 - alpha_bar / alpha_bar_prev)
        ) # 控制随机性的开关，sigma = 1变成标准的DDPM全随机，sigma = 0,变成存粹的DDIM，中间状态都是混合模型
        # Equation 12.
        noise = th.randn_like(x)
        mean_pred = (
        ...
        sample = mean_pred + nonzero_mask * sigma * noise # 构造下一步的图像
        return {"sample": sample, "pred_xstart": out["pred_xstart"]}
    ...
    def ddim_reverse_sample(
        ...
        eps = (
            ...
            - out["pred_xstart"]
        ) / _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x.shape) # 反求噪声
        alpha_bar_next = _extract_into_tensor(self.alphas_cumprod_next, t, x.shape) # 将原图添加噪声
        ...
        return {"sample": mean_pred, "pred_xstart": out["pred_xstart"]} # 当前一步模糊图预测出来的原图 = out["pred_xstart"]
    ...
    # Omnissr版本的ResShift loop
    # 复刻 p_mean_variance_canvas_pano 
    def omnissr_p_mean_variance(
        self, model, x_t, y, t,
        clip_denoised=True,
        denoised_fn=None,
